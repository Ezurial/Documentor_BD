  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          <DropdownMenuItem onClick={onUploud}>
  ...
  const getCurrentHeading = () => {
    for (let level = 1; level <= 5; level++) {
      if (editor?.isActive("heading", { level })) {
        return `Heading ${level}`; // Fixed: using backticks for template literal
      }
    }
    return "Normal text";
  };
...
const ToolbarButton = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "text-sm h-7 min-w-7 flex items-center justifay-center rounded-sm hover:bg-neutral-200/80",
        isActive && "bg-neutral-200/80"
      )}
    >
      <Icon className="size-4" />
...
export const Toolbar = ({ onAiClick }: ToolbarProps) => {
  ...
  // ✅ PASTIKAN return ini ada dalam fungsi, bukan setelah array ditutup
  return (
    <div className="bg-[#eaeff5] px-2.5 py-0.5 rounded-[24px] min-h-[40px] flex items-center gap-x-0.5 overflow-x-auto">
      {/* Tombol-tombol */}
      {sections[0].map((item) => (
